...
const EGYPT_LOCATIONS = {
    ...
    "كفر الشيخ": ["كفر الشيخ", "دسوق", "بلطيم", "البرلس"],
    "الفيوم": ["الفيوم", "سنورس", "طامية", "إطسا"],
    "بني سويف": ["بني سويف", "الواسطى", "ناصر", "ببا"],
    "المنيا": ["المنيا", "ملوي", "سمالوط", "بني مزار", "مغاغة"],
    "أسيوط": ["أسيوط", "ديروط", "القوصية", "منفلوط"],
    "سوهاج": ["سوهاج", "جرجا", "البلينا", "المراغة", "طهطا"],
    "قنا": ["قنا", "نجع حمادي", "دشنا", "قوص"],
    "الأقصر": ["الأقصر", "القرنة", "أرمنت"],
    "أسوان": ["أسوان", "كوم أمبو", "دراو", "إدفو"],
    "شمال سيناء": ["العريش", "بئر العبد", "الشيخ زويد"],
    "جنوب سيناء": ["شرم الشيخ", "دهب", "نويبع", "الطور"],
    "مطروح": ["مرسى مطروح", "الضبعة", "سيدي براني", "السلوم"],
    "البحر الأحمر": ["الغردقة", "مرسى علم", "سفاجا", "رأس غارب"]
};
...
function logout() { G.currentUser = null; saveUserToStorage(); localStorage.removeItem('gymawy_keep_logged_in'); hideMainApp(); T('تم تسجيل الخروج', 'info'); }
...
function showRegForm() {
    var c = document.getElementById('regForm'); if (!c) return;
...
function showResetForm() {
    var c = document.getElementById('resetForm'), rf = document.getElementById('regForm'); if (rf) rf.style.display = 'none'; if (!c) return;
    c.style.display = 'block';
    c.innerHTML = '<h4 style="margin-bottom:12px;">نسيت كلمة المرور</h4>' +
        '<div style="text-align:center;padding:20px;"><i class="fas fa-tools" style="font-size:50px;color:var(--warning);margin-bottom:12px;"></i><h3 style="color:var(--warning);">🚧 تحت التطوير</h3><p>هذه الميزة قيد التطوير</p></div>' +
...
function loadNotifications() {
    ...
    if (G.currentUser && G.currentUser.username) {
        ...
        rtdb.ref('notifications/' + username).on('child_added', function(snap) { var data = snap.val(); if (data && data.title && data.message) addRTDBNotification(snap.key, data); });
        rtdb.ref('users/' + username + '/notifications').on('child_added', function(snap) { var data = snap.val(); if (data && data.title && data.message) addRTDBNotification(snap.key, data); });
    }
    updateBadges();
}
    ...
    try {
        ...
        snap.forEach(function(doc) {
            ...
            if (d.active !== false && d.suspended !== true) {
                G.allGyms.push({ id: doc.id, name: d.name, images: d.images, verified: d.verified, gender: d.gender, governorate: d.governorate, city: d.city, location: d.location, monthlyPrice: d.monthlyPrice, yearlyPrice: d.yearlyPrice, sessionPrice: d.sessionPrice, openTime: d.openTime, closeTime: d.closeTime, phones: d.phones, mapLink: d.mapLink, avgRating: d.avgRating || 0, ratingCount: d.ratingCount || 0, is24h: d.is24h, genderShifts: d.genderShifts });
            }
        });
        if (!G.allGyms.length) { c.innerHTML = '<div class="empty-state"><i class="fas fa-dumbbell"></i><h4>لا توجد جيمات متاحة حالياً</h4></div>'; return; }
...
function displayGyms(gyms) {
    ...
    for (var i = 0; i < gyms.length; i++) {
        ...
        var shiftHtml = '';
        if (g.is24h) {
            shiftHtml = '<span class="shift-tag">24 ساعة</span>';
            ...
            shiftHtml = '<span class="shift-tag">' + (g.openTime||'--') + ' - ' + (g.closeTime||'--') + '</span>';
        }
        
        html += '<div class="gym-card" onclick="showGymDetail(\'' + g.id + '\')">' +
            '<div class="gym-card-header">' + 
            (g.images && g.images.length ? '<img src="' + g.images[0] + '" alt="' + g.name + '">' : 
            '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#667eea,#764ba2);"><i class="fas fa-dumbbell" style="font-size:60px;color:rgba(255,255,255,0.3);"></i></div>') +
            ...
            '<div class="gym-card-overlay"><i class="fas fa-map-marker-alt"></i> ' + (g.governorate || '') + (g.city ? ' • ' + g.city : '') + '</div>' +
            '</div>' +
            ...
            '<div class="price-tag"><div class="price-value">' + (g.sessionPrice || 0) + ' ج.م</div><div class="price-label">حصة</div></div>' +
            '</div>' +
            '<div style="margin:0 14px 8px;font-size:11px;color:' + (g.verified ? 'var(--success)' : 'var(--text-secondary)') + ';">' +
            '<i class="fas fa-' + (g.verified ? 'check-circle' : 'info-circle') + '"></i> ' + (g.verified ? 'جيم موثق من جيماوي' : 'جيم غير موثق') + '</div>' +
            '</div>' +
            '<div class="gym-card-footer"><button class="btn-apply" onclick="event.stopPropagation();applyToGym(\'' + g.id + '\')"><i class="fas fa-hand-paper"></i> تقديم طلب اشتراك</button></div>' +
            '</div>';
    }
    c.innerHTML = html;
}
...
function showGymDetail(gid) {
    ...
    if (gym.is24h) {
        html += '<div class="info-highlight"><i class="fas fa-infinity"></i> مفتوح 24 ساعة</div>';
        ...
        html += '<div class="info-row"><span>يفتح</span><span>' + (gym.openTime || '--') + '</span></div><div class="info-row"><span>يقفل</span><span>' + (gym.closeTime || '--') + '</span></div>';
    }
    html += '</div>';
...
async function submitRating() {
    ...
    var phone = G.currentUser.phone || '';
    var memberSnap = await db.collection('members').where('phone', '==', phone).where('gymId', '==', G.selectedGym.id).get();
    var isMember = false;
    memberSnap.forEach(function(doc) { if (doc.data().active && !doc.data().leftByUser) isMember = true; });
    if (!isMember) { T('فقط المشتركين في الجيم يمكنهم التقييم', 'error'); return; }
...
async function confirmApply() {
    ...
    var amount = gym.monthlyPrice || 0; if (G._subType === 'yearly') amount = gym.yearlyPrice || 0;
...
function displayGymsInContainer(cid, gyms) {
    var c = document.getElementById(cid); if (!c) return; var html = '';
    for (var i = 0; i < gyms.length; i++) { var g = gyms[i]; html += '<div class="gym-card" onclick="showGymDetail(\'' + g.id + '\')" style="margin-bottom:12px;"><div class="gym-card-header">' + (g.images && g.images.length ? '<img src="' + g.images[0] + '">' : '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;"><i class="fas fa-dumbbell ph"></i></div>') + '<div class="gym-card-overlay">' + (g.governorate || '') + '</div></div><div class="gym-card-body"><div class="gym-card-title">' + g.name + '</div><div class="gym-card-prices"><div class="price-tag"><div class="price-value">' + (g.monthlyPrice || 0) + '</div><div class="price-label">ج.م/شهر</div></div></div></div></div>'; }
    c.innerHTML = html;
}
...
function addToRecentlyViewed(gym) { G.recentlyViewed = G.recentlyViewed.filter(function(g) { return g.id !== gym.id; }); G.recentlyViewed.unshift({ id: gym.id, name: gym.name, image: gym.images ? gym.images[0] : null, monthlyPrice: gym.monthlyPrice }); if (G.recentlyViewed.length > 5) G.recentlyViewed = G.recentlyViewed.slice(0, 5); saveRecentlyViewed(); displayRecentGrid(); }
function displayRecentGrid() { var t = document.getElementById('recentTitle'), g = document.getElementById('recentGrid'); if (!G.recentlyViewed.length) { if (t) t.style.display = 'none'; if (g) g.innerHTML = ''; return; } if (t) t.style.display = 'flex'; var html = ''; for (var i = 0; i < G.recentlyViewed.length; i++) { var x = G.recentlyViewed[i]; html += '<div class="recent-card" onclick="showGymDetail(\'' + x.id + '\')">' + (x.image ? '<img src="' + x.image + '">' : '<div style="height:80px;background:linear-gradient(135deg,#667eea,#764ba2);display:flex;align-items:center;justify-content:center;color:white;font-size:20px;"><i class="fas fa-dumbbell"></i></div>') + '<div class="info"><div class="name">' + x.name + '</div><div class="price">' + (x.monthlyPrice || 0) + ' ج.م</div></div></div>'; } if (g) g.innerHTML = html; }
...
async function loadProfile() {
    ...
    var html = '<div class="profile-card"><div class="profile-avatar"><i class="fas fa-user"></i></div>' +
        ...
        (canChange ? '<p style="font-size:10px;color:var(--success);margin-top:8px;">يمكنك تغيير اسم المستخدم</p>' : '<p style="font-size:10px;color:var(--danger);margin-top:8px;">تغيير اسم المستخدم متاح بعد ' + daysUntil + ' يوم</p>') + '</div>';
    
    if (activeMemberships.length) {
        html += '<h3 style="margin-bottom:8px;"><i class="fas fa-dumbbell"></i> اشتراكاتي الحالية</h3>';
        for (var i = 0; i < activeMemberships.length; i++) { var m = activeMemberships[i]; html += '<div class="membership-card"><h4>' + m.gymName + '</h4><div class="info-box"><div class="info-row"><span>النوع</span><span>' + (m.type === 'monthly' ? 'شهري' : 'سنوي') + '</span></div><div class="info-row"><span>البداية</span><span>' + formatDate(m.start) + '</span></div><div class="info-row"><span>الاستحقاق</span><span>' + formatDate(m.due) + '</span></div><div class="info-row"><span>متبقي</span><span>' + daysRemaining(m.due) + ' يوم</span></div></div><span class="status-active">✅ نشط</span></div>'; }
    }
    
    if (pastMemberships.length) {
        html += '<h3 style="margin-bottom:8px;"><i class="fas fa-history"></i> اشتراكات سابقة</h3>';
        for (var j = 0; j < pastMemberships.length; j++) { var pm = pastMemberships[j]; html += '<div class="membership-card" style="opacity:0.7;"><h4>' + pm.gymName + '</h4><div class="info-box"><div class="info-row"><span>النوع</span><span>' + (pm.type === 'monthly' ? 'شهري' : 'سنوي') + '</span></div><div class="info-row"><span>انتهى</span><span>' + formatDate(pm.due) + '</span></div></div><span class="status-expired">❌ منتهي</span></div>'; }
    }
    c.innerHTML = html;
}
...
function showEditAccount() {
    ...
    document.getElementById('modalBody').innerHTML = 
        ...
        '<div class="form-group"><label>كلمة المرور الجديدة (اتركه فاضي لو مش عايز تغير)</label><input type="password" id="editNewPass" class="form-control" placeholder="6 أحرف على الأقل"></div>' +
    ...
    document.getElementById('modalFooter').innerHTML = '<button class="btn btn-primary" onclick="saveAccountChanges()">حفظ التعديلات</button><button class="btn btn-outline" onclick="closeModal(\'gymModal\')">إلغاء</button>';
    openModal('gymModal');
}
...
async function saveAccountChanges() {
    ...
    if (!newPhone || newPhone.length < 10) { T('رقم الهاتف غير صحيح', 'error'); return; }
    if (!newEmail || !newEmail.includes('@')) { T('بريد إلكتروني صحيح مطلوب', 'error'); return; }
    
    var snap = await db.collection('users').doc(G.currentUser.username).get();
    if (!snap.exists) { T('خطأ في تحميل البيانات', 'error'); return; }
    var userData = snap.data();
    if (CryptoJS.SHA256(currentPass).toString() !== userData.passHash) { T('كلمة المرور الحالية غير صحيحة', 'error'); return; }
    ...
    if (newPass && newPass.length >= 6) { updateData.passHash = CryptoJS.SHA256(newPass).toString(); }
    await db.collection('users').doc(G.currentUser.username).update(updateData);
    G.currentUser.phone = newPhone; G.currentUser.email = newEmail;
    saveUserToStorage();
    closeModal('gymModal'); loadProfile();
...
async function changeUsername() {
    if (!G.currentUser) return;
    var lastChange = localStorage.getItem('gymawy_username_last_change');
    if (lastChange) { var daysSince = Math.floor((new Date() - new Date(lastChange)) / 86400000); if (daysSince < 7) { T('لا يمكن تغيير اسم المستخدم قبل ' + (7 - daysSince) + ' يوم', 'error'); return; } }
    document.getElementById('modalTitle').textContent = 'تغيير اسم المستخدم';
    document.getElementById('modalBody').innerHTML = '<div class="form-group"><label>اسم المستخدم الحالي</label><input type="text" class="form-control" value="@' + G.currentUser.username + '" disabled></div><div class="form-group"><label>اسم المستخدم الجديد</label><input type="text" id="newUsername" class="form-control" style="text-transform:uppercase;" placeholder="4-16 حرف"></div><p style="font-size:11px;color:var(--text-secondary);">يمكنك تغيير اسم المستخدم مرة واحدة كل 7 أيام</p>';
    document.getElementById('modalFooter').innerHTML = '<button class="btn btn-primary" onclick="saveNewUsername()">حفظ</button><button class="btn btn-outline" onclick="closeModal(\'gymModal\')">إلغاء</button>';
    openModal('gymModal');
}

async function saveNewUsername() {
    var newUsername = document.getElementById('newUsername')?.value?.trim().toUpperCase();
    if (!newUsername || newUsername.length < 4) { T('اسم المستخدم 4 أحرف على الأقل', 'error'); return; }
    if (newUsername === G.currentUser.username) { T('نفس الاسم الحالي', 'error'); return; }
    var snap = await db.collection('users').doc(newUsername).get();
    if (snap.exists) { T('اسم المستخدم مستخدم بالفعل', 'error'); return; }
    var oldSnap = await db.collection('users').doc(G.currentUser.username).get();
    var userData = oldSnap.data(); userData.username = newUsername;
...
// ==================== SUPPORT ====================
function openSupportChat() { if (!G.currentUser) { T('سجل دخولك', 'warning'); return; } document.getElementById('modalTitle').innerHTML = '<i class="fas fa-headset"></i> خدمة العملاء'; document.getElementById('modalBody').innerHTML = '<div style="text-align:center;"><i class="fas fa-headset" style="font-size:50px;color:var(--primary);"></i><h3>Gymawy Support</h3></div><div class="chat-messages" id="supportChatMessages"><div style="text-align:center;padding:20px;">جاري التحميل...</div></div><div style="display:flex;gap:8px;"><input type="text" id="supMsg" class="form-control" placeholder="اكتب رسالتك..."><button class="btn btn-primary btn-sm" onclick="sendSupMsg()"><i class="fas fa-paper-plane"></i></button></div>'; document.getElementById('modalFooter').innerHTML = '<button class="btn btn-outline" onclick="closeModal(\'gymModal\')">إغلاق</button>'; openModal('gymModal'); loadSupMsgs(); }
async function sendSupMsg() { var m = document.getElementById('supMsg'); if (!m || !G.currentUser) return; var msg = m.value.trim(); if (!msg) return; await db.collection('support_messages').add({ chatId: G.currentUser.username, sender: G.currentUser.username, senderName: G.currentUser.fullName, senderPhone: G.currentUser.phone || '', senderEmail: G.currentUser.email || '', message: msg, timestamp: new Date().toISOString(), read: false }); rtdb.ref('support_notifications').push({ sender: G.currentUser.username, senderName: G.currentUser.fullName, timestamp: new Date().toISOString() }); m.value = ''; loadSupMsgs(); T('تم الإرسال', 'success'); }
async function loadSupMsgs() { var c = document.getElementById('supportChatMessages'); if (!c) return; var s = await db.collection('support_messages').where('chatId', '==', G.currentUser.username).get(); var msgs = []; s.forEach(function(doc) { msgs.push(doc.data()); }); msgs.sort(function(a, b) { return new Date(a.timestamp) - new Date(b.timestamp); }); var html = ''; if (msgs.length) for (var i = 0; i < msgs.length; i++) html += '<div class="chat-bubble ' + (msgs[i].sender === G.currentUser.username ? 'outgoing' : 'incoming') + '">' + msgs[i].message + '<div class="chat-time">' + formatDate(msgs[i].timestamp) + '</div></div>'; else html = '<div style="text-align:center;padding:20px;"><p>اكتب رسالتك</p></div>'; c.innerHTML = html; }

// ==================== ABOUT ====================
function showAbout() { document.getElementById('modalTitle').textContent = 'عن التطبيق'; document.getElementById('modalBody').innerHTML = '<div style="text-align:center;margin-bottom:20px;"><div style="width:80px;height:80px;background:linear-gradient(135deg,var(--primary),var(--secondary));border-radius:20px;display:inline-flex;align-items:center;justify-content:center;font-size:40px;color:white;"><i class="fas fa-dumbbell"></i></div><h2>Gymawy</h2><p style="color:var(--primary);">أول منصة جيمات ذكية في مصر</p></div><div class="info-section"><h4>هدفنا</h4><p>نساعد كل شخص يلاقي الجيم المناسب ليه بكل سهولة - مجاناً.</p></div>'; document.getElementById('modalFooter').innerHTML = '<button class="btn btn-outline" onclick="closeModal(\'gymModal\')">إغلاق</button>'; openModal('gymModal'); }
function showPrivacy() { document.getElementById('modalTitle').textContent = 'سياسة الخصوصية'; document.getElementById('modalBody').innerHTML = '<div class="info-section"><h4>خصوصيتك أمانة</h4><p>نلتزم بحماية بياناتك وعدم مشاركتها.</p></div><div class="info-section"><h4>حذف حسابك</h4><p>يحق لك طلب حذف حسابك من خلال خدمة العملاء.</p></div>'; document.getElementById('modalFooter').innerHTML = '<button class="btn btn-outline" onclick="closeModal(\'gymModal\')">إغلاق</button>'; openModal('gymModal'); }
function showWhoWeAre() { document.getElementById('modalTitle').textContent = 'من نحن'; document.getElementById('modalBody').innerHTML = '<div class="info-section"><h4>Gymawy</h4><p>شركة مصرية ناشئة في التكنولوجيا الرياضية. مقرنا: مصر.</p></div><div class="info-section" style="background:var(--primary-bg);border-radius:12px;padding:14px;text-align:center;"><h4 style="color:var(--primary);">أضف جيمك مجاناً</h4><p>لو عندك جيم وعايز تضمه للمنصة، سجل من هنا.</p><button class="btn btn-primary btn-sm" onclick="showCoachRegistration();closeModal(\'gymModal\');">أضف جيمك مجاناً</button></div>'; document.getElementById('modalFooter').innerHTML = '<button class="btn btn-outline" onclick="closeModal(\'gymModal\')">إغلاق</button>'; openModal('gymModal'); }
...
// ==================== COACH REGISTRATION ====================
const coachReg = { step: 1, name: '', phone: '', email: '', idCardNumber: '', idCardImage: '', governorate: '', city: '', gymName: '', location: '', mapLink: '', phones: [], gymFrontImage: '' };
...
function showCoachRegistration() {
    ...
    document.getElementById('modalBody').innerHTML = '<div style="text-align:center;margin-bottom:12px;"><i class="fas fa-plus-circle" style="font-size:40px;color:var(--success);"></i><h3>أضف جيمك مجاناً</h3><p style="font-size:12px;color:var(--text-secondary);">فريق جيماوي هيتواصل معاك</p></div>' +
        ...
        '<div class="form-group"><label>العنوان التفصيلي *</label><input type="text" id="crLocation" class="form-control" placeholder="شارع، منطقة، أقرب علامة"></div>' +
        '<div class="form-group"><label>رابط Google Maps</label><input type="text" id="crMapLink" class="form-control" placeholder="رابط موقع الجيم"></div>' +
    ...
    document.getElementById('modalFooter').innerHTML = '<button class="btn btn-success" onclick="submitCoachReg()">تقديم الطلب</button><button class="btn btn-outline" onclick="closeModal(\'gymModal\')">إلغاء</button>';
    openModal('gymModal');
}
...
async function submitCoachReg() {
    ...
    var gymName = document.getElementById('crGymName')?.value?.trim(), governorate = document.getElementById('crGovernorate')?.value, city = document.getElementById('crCity')?.value;
    var location = document.getElementById('crLocation')?.value?.trim(), mapLink = document.getElementById('crMapLink')?.value?.trim();
    var phone1 = document.getElementById('crPhone1')?.value?.trim(), phone2 = document.getElementById('crPhone2')?.value?.trim();
    var gymFile = document.getElementById('crGymImage')?.files[0];
    if (!name || !phone || !email || !idNumber || !idFile || !gymName || !governorate || !city || !location || !phone1 || !gymFile) { T('اكمل كل البيانات المطلوبة *', 'error'); return; }
    
    var idFormData = new FormData(); idFormData.append('file', idFile); idFormData.append('upload_preset', 'gymawy_upload');
    var idUrl = ''; try { var idRes = await fetch('https://api.cloudinary.com/v1_1/di5z4lzwv/image/upload', { method: 'POST', body: idFormData }); var idData = await idRes.json(); idUrl = idData.secure_url || ''; } catch (e) { T('فشل رفع صورة البطاقة', 'error'); return; }
    
    var gymFormData = new FormData(); gymFormData.append('file', gymFile); gymFormData.append('upload_preset', 'gymawy_upload');
    var gymImageUrl = ''; try { var gymRes = await fetch('https://api.cloudinary.com/v1_1/di5z4lzwv/image/upload', { method: 'POST', body: gymFormData }); var gymData = await gymRes.json(); gymImageUrl = gymData.secure_url || ''; } catch (e) { T('فشل رفع صورة الجيم', 'error'); return; }
    
    var phones = [phone1]; if (phone2) phones.push(phone2);
    await db.collection('coach_applications').add({ name, phone, email, idCardNumber: idNumber, idCardImage: idUrl, gymName, governorate, city, location, mapLink, phones, gymFrontImage: gymImageUrl, status: 'pending', createdAt: new Date().toISOString() });
    closeModal('gymModal');
    T('تم تقديم الطلب - فريق جيماوي هيتواصل معاك خلال 48 ساعة', 'success');
}
...
console.log('🏋️ Gymawy Ready - Free App');